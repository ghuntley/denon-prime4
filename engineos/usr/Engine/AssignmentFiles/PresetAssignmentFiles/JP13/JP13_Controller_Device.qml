...
Device {
	...
	function sendInitializationMessage() {
		Midi.sendSysEx("F0 00 02 0B 00 0D 04 00 00 F7")
	}
	...
	function sendColor(channel, index, color)
	{
		...
		const sysEx = "F0 00 02 0B 7F 0D 03 00 04 " + d2h(index) + " " + midiColor(color, g) + " F7"
		Midi.sendSysEx(sysEx)

	}

	function requestPowerOnButtonState() {
		Midi.sendSysEx("F0 00 02 0B 01 0D 42 00 00 F7")
	}
