...
Device {
	...
	readonly property QObjProperty pRunningDark: Planck.getProperty("/GUI/Scripted/RunningDark");
	readonly property bool isRunningDark: pRunningDark && pRunningDark.translator ? pRunningDark.translator.state : false

	onIsRunningDarkChanged: {
		if(isRunningDark) {
			Midi.sendSysEx("F0 00 02 0B %1 0B 7C 00 01 00 F7".arg(device.d2h(midiDeviceId)))
		}
		else {
			Midi.sendSysEx("F0 00 02 0B %1 0B 7C 00 01 7F F7".arg(device.d2h(midiDeviceId)))
		}
	}

	readonly property string side: midiDeviceId === 0x10 ? "Left" : "Right"
	property int midiDeviceId: deviceIdFromInquiry();

	function deviceIdFromInquiry() {
		var l = deviceInquiryResponse.split(" ");
		if(l.length >= 15) {
			return parseInt(l[15], 16)
		}
	}
	...
	readonly property QtObject slipPosition: QtObject {
		...
		onIndicatorColorChanged: {
			device.setElementARGB(5, indicatorColor);
		}
	...
	function setElementARGB(elementId, color) {
		...
		Midi.sendSysEx("F0 00 02 0B %1 0B 0B 00 09 %2 %3 F7".arg(device.d2h(midiDeviceId)).arg(device.d2h(elementId)).arg(colorToHex(color)))
	}
	...
	function updateDisplayElements() {
		...
		var sysEx = "F0 00 02 0B %1 0B 0A 00 04 ".arg(device.d2h(midiDeviceId))
	...
	readonly property QObjProperty pUpgradeAction: Planck.createProperty("Upgrade", 3);
	readonly property bool doUpgrade: pUpgradeAction && pUpgradeAction.translator ? pUpgradeAction.translator.state : false

	function sysEx(sysExString) {
		var valueList = sysExString.split(" ");
		var result = "";

		var code = parseInt(valueList[6], 16)

		if(code === 0x0f) {
			var errCode = parseInt(valueList[9], 16);
			if(errCode !== 0x00) {
				console.warn("Upload failed:", errCode)
			}
		}
		else {
			for(var i=0;i<4;i++) {
			...
			if(valueList.length >= 15) {
				midiDeviceId = parseInt(valueList[15], 16)
			}
			deviceInfo = result;
		}
	}
	...
	Component.onCompleted: {
		...
		Midi.sendSysEx("F0 00 02 0B %1 0B 10 00 00 F7".arg(device.d2h(midiDeviceId))) // Start decoding images
	}
