...
MidiAssignment {
	id: assignment
	objectName: "PRIME2 Wheel Display (" + device.side + ")"

	readonly property int deckIndex: device.side === "Left" ? 1 : 2
	...
	Item {
		Painter {
			...
			function draw() {
				...
				var data = imageToByteArrays(assignment.hasCurrentDeviceArtwork ? deviceArtworkIndex : 1, 0x0B, device.midiDeviceId)
	...
	Item {
		...
		readonly property QObjProperty pSongLoaded: Planck.getProperty("/Engine/Deck%1/Track/SongLoaded".arg(assignment.deckIndex))
		readonly property bool songLoaded: pSongLoaded && pSongLoaded.translator ? pSongLoaded.translator.state : false

		function show() {
			assignment.isSwitchingLayer = true
			device.logo.enabled = false
			device.loopAndLayerText.text = assignment.deckIndex
			device.loopAndLayerText.enabled = true
			device.artwork.alpha = 0.5
		}
	...
	OutputAssignment {
		...
		function send() {
			...
			Midi.sendSysEx("F0 00 02 0B %1 0B 7c 00 01 %2 F7".arg(device.d2h(device.midiDeviceId)).arg(device.d2h(brightness)))
		}
