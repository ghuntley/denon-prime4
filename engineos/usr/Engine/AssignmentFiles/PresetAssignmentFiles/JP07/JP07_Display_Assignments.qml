...
MidiAssignment {
		...
		Item {
			Painter {
				...
				function draw() {
					...
					var data = imageToByteArrays(assignment.hasCurrentDeviceArtwork ? deviceArtworkIndex : 1 + index, 0x06, 0x01)
	...
	OutputAssignment {
		...
		function send() {
			...
			Midi.sendSysEx("F0 00 02 0B 01 06 7c 00 01 %1 F7".arg(device.d2h(brightness)))
		}
