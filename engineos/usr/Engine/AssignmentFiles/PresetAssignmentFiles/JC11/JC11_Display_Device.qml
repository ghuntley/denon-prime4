...
Device {
	...
	onIsRunningDarkChanged: {
		if(isRunningDark) {
			Midi.sendSysEx("F0 00 02 0B %1 08 7C 00 01 00 F7".arg(device.d2h(midiDeviceId)))
		}
		else {
			Midi.sendSysEx("F0 00 02 0B %1 08 7C 00 01 7F F7".arg(device.d2h(midiDeviceId)))
		}
	}

	property string side: midiDeviceId === 0x10 ? "Left" : "Right"
	...
	function setElementARGB(elementId, color) {
		...
		Midi.sendSysEx("F0 00 02 0B %1 08 0B 00 09 %2 %3 F7".arg(device.d2h(midiDeviceId)).arg(device.d2h(elementId)).arg(colorToHex(color)))
	}
	...
	function updateDisplayElements() {
		...
		var sysEx = "F0 00 02 0B %1 08 0A 00 04 ".arg(device.d2h(midiDeviceId))
	...
	property QObjProperty pUpgradeAction: Planck.createProperty("Upgrade", 3);
	property bool doUpgrade: pUpgradeAction && pUpgradeAction.translator ? pUpgradeAction.translator.state : false
	...
	function sysExToIntList(sysExString)
	{
		...
		for(var val in valueList) {
			result.push(parseInt(valueList[val], 16))
		}
	...
	function sysEx(sysExString) {
		...
		if(code === 0x10) {
			console.log("Image decoding finished", sysExString)
			imageSendingEnabled = true
			updateDisplayElements()
		}
		else if(code === 0x0f) {
			var errCode = valueList[9];
			if(errCode !== 0x00) {
	...
	property Timer imageSendingEnableTimeout: Timer {
		interval: 17000
		repeat: false
		onTriggered: {
			if(!device.imageSendingEnabled) {
				device.imageSendingEnabled = true
				device.updateDisplayElements()
			}
		}
	}
	...
	Component.onCompleted: {
		...
		console.log("Start decoding images")
		imageSendingEnableTimeout.start()
		Midi.sendSysEx("F0 00 02 0B %1 08 10 00 00 F7".arg(device.d2h(midiDeviceId))) // Start decoding images
	}
