...
MidiAssignment {
	id: assignment
	objectName: "PRIME4 Wheel Display (" + device.side + ")"

	readonly property string activeDeckIndex: Planck.getProperty("/GUI/Decks/Deck%1/ActiveDeck".arg(device.side)).translator.string
	...
	property int activeLayer: (parseInt(activeDeckIndex) > 2) ? 1 : 0

	readonly property QObjProperty pDeckColor: Planck.getProperty("/Engine/Deck%1/JogColor".arg(activeDeckIndex))
	property color deckColor: pDeckColor && pDeckColor.translator ? pDeckColor.translator.color : Qt.rgba(1,1,1,1)

	property bool isLineIn: Planck.getProperty("/Engine/Mixer/Channel%1/Line".arg(activeDeckIndex)).translator.state

	onDeckColorChanged: {
		device.deckColor = deckColor
	}

	Repeater {
		model: device.side === "Left" ? leftModel : rightModel
		...
		Item {
			Painter {
				...
				readonly property QObjProperty pAlbumArt: Planck.getProperty("/Engine/Deck%1/AlbumArt".arg(deckIndex))
				...
				property bool isSelectedLayer: parseInt(assignment.activeDeckIndex) === deckIndex

				onHasValidArtworkChanged: updateArtworkSettingsOnDevice()
				onIsSelectedLayerChanged: updateArtworkSettingsOnDevice()
				...
				readonly property bool isLineIn: assignment.isLineIn
				onIsLineInChanged: updateArtworkSettingsOnDevice()
				...
				function updateArtworkSettingsOnDevice() {
					...
					if(isSelectedLayer) {
						...
						else if(assignment.hasCurrentDeviceArtwork) {
				...
				function draw() {
					...
					var data = imageToByteArrays(assignment.hasCurrentDeviceArtwork ? deviceArtworkIndex : 1 + index, 0x08, device.midiDeviceId)
	...
	Item {
		...
		readonly property bool platterPositionEnabled: assignment.songLoaded && !assignment.isLineIn
		...
		Timer {
			...
			property var positionItems: [
				Planck.createPositionItem("/Private/Deck%1/MidiSamplePosition".arg(device.side === "Left" ? 1 : 2)),
				Planck.createPositionItem("/Private/Deck%2/MidiSamplePosition".arg(device.side === "Left" ? 3 : 4))
			]
			...
			onTriggered: {
				var posItem = positionItems[assignment.activeLayer]
	...
	Item {
		...
		readonly property bool slipModeEnabled: device.slipPosition.active = slipModeActive && assignment.songLoaded  && !assignment.isLineIn
	...
	Item {
		...
		readonly property bool loopEnabled: loopEnableState && !assignment.isSwitchingLayer && !assignment.isLineIn
	...
	Item {
		...
		onIsLineInChanged:{
			if(assignment.isLineIn) {
				device.logo.enabled = true
			}
			else {
	...
	Item {
		...
		readonly property int viewLayer: assignment.activeLayer

		readonly property QObjProperty pSongLoaded: Planck.getProperty("/Engine/Deck%1/Track/SongLoaded".arg(assignment.activeDeckIndex))
		readonly property bool songLoaded: pSongLoaded && pSongLoaded.translator ? pSongLoaded.translator.state : false
		...
		function show() {
			...
			device.loopAndLayerText.text = assignment.activeDeckIndex
			device.loopAndLayerText.enabled = true
			device.artwork.alpha = 0.5
		}
	...
	OutputAssignment {
		...
		function send() {
			...
			Midi.sendSysEx("F0 00 02 0B %1 08 7c 00 01 %2 F7".arg(device.d2h(device.midiDeviceId)).arg(device.d2h(brightness)))
		}
